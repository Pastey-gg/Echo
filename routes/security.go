...
func (v *SecurityView) LoadRoutes() {
	...
	v.ctx.Server.DELETE("/security/:token/files/:file_id", v.deleteFile)
...
func (v *SecurityView) deleteFile(c *echo.Context) error {
	token := c.Param("token")
	fileID := c.Param("file_id")

	if err := v.ctx.Database.DeleteFile(token, fileID); err != nil {
		switch {
		case errors.Is(err, models.ErrNotFound):
			return c.JSON(http.StatusNotFound, map[string]string{"error": "Token or file not found."})
		...
		v.ctx.Logger.Errorf("Failed to delete file %s for token %s: %v", fileID, token, err)
		return c.JSON(http.StatusInternalServerError, map[string]string{"error": "Internal server error."})
	}
